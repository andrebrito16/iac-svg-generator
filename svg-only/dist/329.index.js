...
const generateSVG = (grades) => {
    ...
    grades.forEach((grade, index) => {
        ...
        let activityText = grade.activity;
        if (activityText.length > 14) {
            activityText = activityText.substring(0, 11) + "...";
        }
        ...
        svg += `<text x="${x + badgeWidth * 0.25}" y="${y + badgeHeight * 0.75}" fill="#010101" fill-opacity=".3">${activityText}</text><text x="${x + badgeWidth * 0.25}" y="${y + badgeHeight * 0.70}">${activityText}</text>`;
