export const generateSVG = (grades: Grade[]): string => {
  ...
  grades.forEach((grade, index) => {
    ...
    if (activityText.length > 20) {
      activityText = activityText.substring(0, 17) + "...";
    }
    ...
    svg += `<text x="${x + badgeWidth * 0.25}" y="${y + badgeHeight * 0.75}" fill="#010101" fill-opacity=".3">${activityText}</text><text x="${x + badgeWidth * 0.25}" y="${y + badgeHeight * 0.70}">${grade.activity}</text>`;
